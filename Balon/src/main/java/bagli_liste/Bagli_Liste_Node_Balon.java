...
public class Bagli_Liste_Node_Balon {
	...
	private Node_Balon yeni_balon(int no) {
		...
		try {
			this.bagli_liste_node_balon.getYeniBalon().setcurrentNode(no, Renkler.random_renk());
		} catch (NullPointerException e) {
			e.printStackTrace();
			System.exit(0);
			return null;
	...
	public void Ekle(int no) {
		if (this.bagli_liste_node_balon == null && this.bagli_liste_node_balon.getBas() == null
				&& this.bagli_liste_node_balon.getBas().getNode_balon() == null) {
			System.exit(0);
		} else {
			if (this.bagli_liste_node_balon.getBas().getNode_balon().getNode() == null) {
				Balon balon = new Balon(no, Renkler.random_renk());
				this.bagli_liste_node_balon.getBas().getNode_balon().setcurrentNode(balon);
				...
				do {
					try {
						this.bagli_liste_node_balon.getGecici().sonraki_node_balonu_simdiki_node_balon_olarak_ayarla();
					} catch (NullPointerException e) {
						e.printStackTrace();
						System.exit(0);
						return;
				...
				} while (this.bagli_liste_node_balon.getSimdiki().getNode_balon() != this.bagli_liste_node_balon
	...
	public String toString() {
		...
		s.append(this.bagli_liste_node_balon);
		s.append("]");
		return s.toString();
	}
	...
	public void patlat(int n) {
		...
		if (this.bagli_liste_node_balon.getBas() == null) {
			...
			if (this.bagli_liste_node_balon.getBas().getNode_balon() == null) {
				...
				if (this.bagli_liste_node_balon.getBas().getNode_balon().getNode() == null) {
					...
					try {
						this.bagli_liste_node_balon.getSilinecek().setNode_balon(
								this.bagli_liste_node_balon.getBas().getSonraki_node_balon());/* silinecek olan balon */
					} catch (NullPointerException e) {
						e.printStackTrace();
						System.exit(0);
						return;
		...
		if (n == 1) {// eger balonlari teker teker silmemiz isteniyorsa
			while (this.bagli_liste_node_balon.getSilinecek().getSonraki_node_balon() != this.bagli_liste_node_balon
					.getBas().getNode_balon()) {
				this.bagli_liste_node_balon.getSilinecek().sonraki_node_balonu_simdiki_node_balon_olarak_ayarla();
			}
			s.append(this.bagli_liste_node_balon.getKatNumarasi());
			s.append(".kattaki ");
			s.append(n);
			...
			while (doluMu()) {
				s.append("\nSilinen; ");
				s.append(this.bagli_liste_node_balon.Bas_toString());
			...
			s.append("\n" + this.bagli_liste_node_balon.getKatNumarasi() + ".katta ilk balondan itibaren her " + n
					+ ".balon patlatilmaktadir.\n\n");
			while (doluMu()) {
				s.append(this.bagli_liste_node_balon.getKatNumarasi() + ".katta kalan balonlar:");
				s.append(this.dolas());
				...
				s.append(this.bagli_liste_node_balon.Silinecek_toString()
		...
		s.append(this.bagli_liste_node_balon.Bas_toString());/* onu da ekrana yazdiralim. */
	...
	public String dolas() {
		...
		if (this.bagli_liste_node_balon == null && this.bagli_liste_node_balon.getBas() == null
				&& this.bagli_liste_node_balon.getBas().getNode_balon() == null
				&& this.bagli_liste_node_balon.getBas().getNode_balon().getNode() == null) {
			s.append("Liste bos");
		} else {
			this.bagli_liste_node_balon.setDolas(this.bagli_liste_node_balon.getBas());
			do {
				try {
					s.append(this.bagli_liste_node_balon.Dolas_toString() + "\n");
				} catch (NullPointerException e) {
					e.printStackTrace();
					System.exit(0);
				} catch (Exception e) {
